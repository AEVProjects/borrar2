...
DROP TABLE IF EXISTS public.apollo_leads;
...
CREATE INDEX idx_apollo_leads_email ON public.apollo_leads(email);
CREATE INDEX idx_apollo_leads_company ON public.apollo_leads(company_name);
CREATE INDEX idx_apollo_leads_industry ON public.apollo_leads(industry);
CREATE INDEX idx_apollo_leads_stage ON public.apollo_leads(stage);

-- Allow public access (adjust RLS as needed)
ALTER TABLE public.apollo_leads ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Allow all access to apollo_leads" ON public.apollo_leads FOR ALL USING (true) WITH CHECK (true);
