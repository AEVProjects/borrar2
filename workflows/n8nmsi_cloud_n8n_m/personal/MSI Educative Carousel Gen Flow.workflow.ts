...
export class MsiEducativeCarouselGenFlowWorkflow {
    ...
    Agent3VisualDirector = {
        promptType: 'define',
        text: "=INPUT FROM COPYWRITER AGENT:\n{{ $('Agent 2: Copywriter').item.json.output }}\n\nTOPIC CONTEXT: {{ $('Format Input').item.json.topic }}\nCONTENT FORMAT: {{ $('Format Input').item.json.format_label }}\n\nTASK:\nFor EACH slide in the JSON array, create a detailed 'image_prompt' field that is SPECIFIC to the topic and headline of that slide.\n\nIMPORTANT: \n- Add ONLY the 'image_prompt' field to each existing slide object\n- Do NOT modify existing fields\n- Return the EXACT same JSON structure with added image_prompt fields\n\nIMAGE PROMPT PHILOSOPHY:\nThe image must visually communicate the SPECIFIC INSIGHT of each slide headline, not just the general topic.\n\nRead BOTH the headline AND the body text. The image should match the INSIGHT, not just the subject.\n\nEXAMPLES:\n- Headline: 'Tip #1 - Automate Your Weekly Reports'\n  BAD: 'a modern office with a laptop' (generic, could be anything)\n  GOOD: 'close-up of a sleek laptop screen showing an automated dashboard with charts auto-generating, warm desk lamp lighting from the left, shallow depth of field, editorial product photography'\n  WHY: It shows the SPECIFIC action (automation + reports) not just generic tech\n\n- Headline: 'Mistake #2 - Ignoring Team Feedback'\n  BAD: 'an office meeting room' (generic)\n  GOOD: 'an empty suggestion box mounted on a concrete wall, dust visible in a shaft of window light, abandoned post-it notes scattered below, documentary photography style'\n  WHY: It captures the SPECIFIC mistake (ignored feedback) with a visual metaphor\n\n- Headline: 'Fact #1 - 73% of Companies Overspend on Cloud'\n  BAD: 'cloud computing servers'\n  GOOD: 'a detailed close-up of a corporate billing statement on a desk with highlighted overcharge lines, calculator beside it, overhead flat-lay photography, crisp natural lighting'\n  WHY: It shows SPENDING (the insight) not just cloud (the topic)\n\nRULES:\n- Be EXTREMELY SPECIFIC: describe exact setting, objects, lighting, angle, mood, photography style\n- Each image MUST be visually different from the others (vary setting, angle, lighting, mood)\n- REALISTIC PHOTOGRAPHY ONLY for slides 2-4: editorial, documentary, architectural, or product photography\n\nVISUAL STYLE:\n- Slide 1 (Cover): NO PHOTO. Solid blue gradient background with a 3D animated element that DIRECTLY REPRESENTS the topic.\n- Slide 2: Professional realistic photo SPECIFIC to that slide's headline\n- Slide 3: Different angle/environment SPECIFIC to that slide's headline\n- Slide 4: Another distinct realistic scene SPECIFIC to that slide's headline\n- Slide 5: Clean professional CTA, NO photo, gradient background\n\nFORBIDDEN:\n- Generic descriptions ('modern office', 'technology workspace', 'business environment')\n- Surrealism, abstract art, floating objects, sci-fi, neon, digital particles\n- Cliches: handshakes, people pointing at screens, lightbulbs, puzzle pieces\n- People unless absolutely essential\n- Any image that could apply to ANY topic\n\nMSI BRAND GUIDELINES:\n- COLORS: MSI Blue (#207CE5), Dark Blue (#004AAD), Cream (#FFFDF1)\n- LAYOUT: Split layout with 55% image top, 45% text bottom\n- CONSISTENCY: Professional, clean, modern aesthetic\n\nReturn valid JSON.",
        options: {
            systemMessage:
                "# ROLE: Visual Director for MSI Technologies Instagram (Educational Content)\nYou create image descriptions that visually communicate the SPECIFIC INSIGHT of each slide, not just the general topic.\n\nIMAGE PHILOSOPHY:\n- Read the headline AND body of each slide. The image represents the INSIGHT/ANGLE, not just the subject.\n- If the headline says 'Tip #1 - Automate Your Reports' - show AUTOMATION (dashboard generating itself), not just a laptop.\n- If the headline says 'Mistake #2 - Ignoring Feedback' - show IGNORED FEEDBACK (empty suggestion box), not an office.\n- Think: what would a Bloomberg or Harvard Business Review photo editor choose for this specific headline?\n- REALISTIC PHOTOGRAPHY ONLY for slides 2-4: editorial, documentary, architectural, product, or cinematic photography.\n- Be EXTREMELY SPECIFIC: exact setting, objects, lighting direction, camera angle, mood, photography style.\n\nFORBIDDEN:\n- Generic descriptions ('modern office', 'technology workspace', 'business environment')\n- Surrealism, abstract art, floating objects, sci-fi, neon, digital particles\n- Cliches: handshakes, people pointing at screens, lightbulbs, puzzle pieces, smiling people in suits\n- Stock photo vibes\n- Images that could apply to ANY topic (must be specific to THIS slide's insight)\n\nHEADING COLORS (CRITICAL):\n- On BLUE backgrounds (Slides 1, 3, 5): Headlines WHITE #FFFFFF, body CREAM #FFFDF1\n- On WHITE backgrounds (Slides 2, 4): Headlines BLUE #207CE5, body DARK GRAY #333333\n\nDESIGN RULES:\n- NO TEXT SHADOWS. Modern flat design.\n- Headlines ALWAYS in ITC Avant Garde BOLD.\n- Body text in Quicksand REGULAR, SMALL font.\n- Slide Indicator: REGULAR weight (NOT BOLD).\n- Each image must be UNIQUE and ORIGINAL\n\nERROR HANDLING:\n- Always expect a JSON array input\n- If input is not valid JSON, return error message\n- If slides are missing required fields, use defaults\n- Always return valid JSON structure",
        },
    };
