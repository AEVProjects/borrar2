import { workflow, node, links } from '@n8n-as-code/transformer';

// <workflow-map>
// Workflow : Daily Content Auto-Generator
// Nodes   : 18  |  Connections: 11
//
// NODE INDEX
// ──────────────────────────────────────────────────────────────────
// Property name                    Node type (short)         Flags
// DailySchedule8amMonFri             scheduleTrigger
// ManualTriggerWebhook               webhook
// PrepareDailyContext                code
// MergeTriggers                      merge
// AiTopicGenerator                   agent                      [AI]
// ParseAiOutput                      code
// SaveScheduledPostToDb              postgres                   [creds]
// CallContentGenerationFlow          httpRequest
// ProcessGenerationResponse          code
// UpdatePostStatusInDb               postgres                   [creds]
// FormatSuccessResponse              set
// RespondToWebhook                   respondToWebhook
// Documentation                      stickyNote
// TriggerZone                        stickyNote
// AiZone                             stickyNote
// DbZone                             stickyNote
// ResponseZone                       stickyNote
// GoogleGeminiChatModel              lmChatGoogleGemini         [creds]
//
// ROUTING MAP
// ──────────────────────────────────────────────────────────────────
// DailySchedule8amMonFri
//    → MergeTriggers
//      → PrepareDailyContext
//        → AiTopicGenerator
//          → ParseAiOutput
//            → SaveScheduledPostToDb
//              → CallContentGenerationFlow
//                → ProcessGenerationResponse
//                  → UpdatePostStatusInDb
//                    → FormatSuccessResponse
//                      → RespondToWebhook
// ManualTriggerWebhook
//    → MergeTriggers (↩ loop)
//
// AI CONNECTIONS
// GoogleGeminiChatModel.uses({ ai_languageModel: AiTopicGenerator })
// </workflow-map>

// =====================================================================
// METADATA DU WORKFLOW
// =====================================================================

@workflow({
    id: 'NShpX3bbm86nHkYs',
    name: 'Daily Content Auto-Generator',
    active: false,
    settings: { executionOrder: 'v1' },
})
export class DailyContentAutoGeneratorWorkflow {
    // =====================================================================
    // CONFIGURATION DES NOEUDS
    // =====================================================================

    @node({
        name: 'Daily Schedule (8AM Mon-Fri)',
        type: 'n8n-nodes-base.scheduleTrigger',
        version: 1.2,
        position: [2032, -48],
    })
    DailySchedule8amMonFri = {
        rule: {
            interval: [
                {
                    field: 'cronExpression',
                    expression: '0 8 * * 1-5',
                },
            ],
        },
    };

    @node({
        name: 'Manual Trigger Webhook',
        type: 'n8n-nodes-base.webhook',
        version: 2,
        position: [2032, 144],
    })
    ManualTriggerWebhook = {
        httpMethod: 'POST',
        path: 'daily-content-trigger',
        responseMode: 'responseNode',
        options: {},
    };

    @node({
        name: 'Prepare Daily Context',
        type: 'n8n-nodes-base.code',
        version: 2,
        position: [2480, 48],
    })
    PrepareDailyContext = {
        jsCode: "// Prepare daily context\nconst now = new Date();\nconst input = $input.item.json;\n\n// Parse body if coming from webhook\nlet data = {};\nif (input.body) {\n  if (typeof input.body === 'string') {\n    try {\n      data = JSON.parse(input.body);\n    } catch (e) {\n      data = input;\n    }\n  } else {\n    data = input.body;\n  }\n} else {\n  data = input;\n}\n\n// Date information\nconst dayOfWeek = now.toLocaleDateString('en-US', { weekday: 'long' });\nconst month = now.toLocaleDateString('en-US', { month: 'long' });\nconst dayOfMonth = now.getDate();\nconst year = now.getFullYear();\nconst quarter = Math.ceil((now.getMonth() + 1) / 3);\n\n// Check for special dates/events\nconst specialDates = {\n  '01-01': 'New Year - Fresh start, new goals',\n  '02-14': 'Valentine\\'s Day - Love your technology',\n  '03-14': 'Pi Day - Mathematical precision in tech',\n  '04-22': 'Earth Day - Sustainable technology',\n  '05-01': 'May Day - Worker productivity tools',\n  '07-04': 'Independence Day - Digital freedom',\n  '10-31': 'Halloween - Scary tech problems solved',\n  '11-25': 'Thanksgiving - Grateful for technology',\n  '12-25': 'Christmas - Tech gifts and year review'\n};\n\nconst dateKey = `${String(now.getMonth() + 1).padStart(2, '0')}-${String(dayOfMonth).padStart(2, '0')}`;\nconst specialEvent = specialDates[dateKey] || null;\n\n// Industry trends for 2026\nconst trendTopics = [\n  'AI-powered automation and business intelligence',\n  'Hybrid cloud infrastructure optimization',\n  'Cybersecurity best practices',\n  'Digital transformation strategies',\n  'IT infrastructure modernization',\n  'Data analytics and business insights',\n  'Remote work technology solutions',\n  'Network optimization and performance',\n  'Enterprise software integration',\n  'Cloud migration strategies'\n];\n\n// Content themes by day of week\nconst dailyThemes = {\n  'Monday': 'Week kickoff - productivity and planning',\n  'Tuesday': 'Technical deep-dive and how-to content',\n  'Wednesday': 'Industry insights and trends',\n  'Thursday': 'Case studies and success stories',\n  'Friday': 'Week wrap-up and weekend tips'\n};\n\nconst dailyTheme = dailyThemes[dayOfWeek] || 'General tech content';\n\n// Visual styles to rotate\nconst visualStyles = ['Infographic', 'Glassmorphism', 'Modern 3D', 'Isometric', 'Data Hero'];\nconst visualStyleIndex = dayOfMonth % visualStyles.length;\nconst suggestedVisualStyle = visualStyles[visualStyleIndex];\n\n// Post types to rotate\nconst postTypes = ['Educational', 'Tips & Tricks', 'Industry News', 'Success Story', 'Problem/Solution'];\nconst postTypeIndex = (dayOfMonth + 2) % postTypes.length;\nconst suggestedPostType = postTypes[postTypeIndex];\n\n// Number of posts to generate (from input or default 1)\nconst postsToGenerate = data.posts_count || 1;\nconst customTopic = data.topic || null;\nconst customVisualStyle = data.visual_style || null;\nconst customPostType = data.post_type || null;\n\nreturn [{\n  json: {\n    date: now.toISOString().split('T')[0],\n    day_of_week: dayOfWeek,\n    month: month,\n    day_of_month: dayOfMonth,\n    year: year,\n    quarter: quarter,\n    special_event: specialEvent,\n    daily_theme: dailyTheme,\n    trend_topics: trendTopics,\n    suggested_visual_style: customVisualStyle || suggestedVisualStyle,\n    suggested_post_type: customPostType || suggestedPostType,\n    posts_to_generate: postsToGenerate,\n    custom_topic: customTopic,\n    timestamp: now.toISOString()\n  }\n}];",
    };

    @node({
        name: 'Merge Triggers',
        type: 'n8n-nodes-base.merge',
        version: 3,
        position: [2256, 48],
    })
    MergeTriggers = {};

    @node({
        name: 'AI Topic Generator',
        type: '@n8n/n8n-nodes-langchain.agent',
        version: 1.7,
        position: [2704, 48],
    })
    AiTopicGenerator = {
        promptType: 'define',
        text: "=Generate a LinkedIn content topic for MSI Technologies.\n\n## TODAY'S CONTEXT\n- Date: {{ $json.date }} ({{ $json.day_of_week }})\n- Month: {{ $json.month }} {{ $json.year }}\n- Quarter: Q{{ $json.quarter }}\n- Daily Theme: {{ $json.daily_theme }}\n{{ $json.special_event ? '- Special Event: ' + $json.special_event : '' }}\n{{ $json.custom_topic ? '- REQUESTED TOPIC: ' + $json.custom_topic : '' }}\n\n## TRENDING INDUSTRY TOPICS\n{{ $json.trend_topics.join('\\n- ') }}\n\n## REQUIREMENTS\n1. Topic must be relevant to IT services and technology consulting\n2. Should appeal to business decision-makers (CTOs, IT Directors, CEOs)\n3. Must be actionable and provide value\n4. Align with the daily theme: {{ $json.daily_theme }}\n{{ $json.custom_topic ? '5. MUST incorporate the requested topic: ' + $json.custom_topic : '' }}",
        options: {
            systemMessage:
                '# ROLE: Content Strategist for MSI Technologies\n\nYou are a content strategist specializing in B2B technology content for LinkedIn.\n\n## OUTPUT FORMAT (STRICT - Return ONLY this format, no extra text)\n\n---TOPIC---\n[One clear, specific topic title - max 10 words, actionable and specific]\n\n---HEADLINE---\n[SHORT attention-grabbing headline for the image - EXACTLY 4-7 words, NO punctuation at end, impactful]\nExample: "Cloud Migration Made Simple" or "Stop Paying for Downtime"\n\n---DATA_POINTS---\n[2-3 compelling statistics or facts, each on a new line with bullet point]\n• [Stat 1]\n• [Stat 2]\n• [Stat 3 optional]\n\n---CONTEXT---\n[Brief context about why this topic matters today - 2-3 sentences max]\n\n## MSI TECHNOLOGIES CONTEXT\nMSI Technologies is a technology consulting company that helps businesses:\n- Modernize IT infrastructure\n- Implement cloud solutions\n- Optimize network performance\n- Enhance cybersecurity\n- Enable digital transformation\n\n## RULES\n1. Topics must be SPECIFIC, not generic (e.g., "Reduce Server Costs 40%" not "Cost Optimization")\n2. Headlines MUST be 4-7 words, punchy, no periods or colons\n3. Data points should be realistic industry statistics\n4. Content should position MSI as thought leaders\n5. NO sales pitches - focus on education and value\n6. If a custom topic is provided, MUST use it as the main focus\n7. Headlines should create urgency or promise specific value\n\n## HEADLINE EXAMPLES (FOLLOW THIS STYLE)\n- "Your Network Is Bleeding Money"\n- "Cloud Migration in 30 Days"\n- "Stop Firefighting Start Automating"\n- "Legacy Systems Cost You Millions"\n- "Cybersecurity Threats You\'re Ignoring"',
        },
    };

    @node({
        name: 'Parse AI Output',
        type: 'n8n-nodes-base.code',
        version: 2,
        position: [3056, 48],
    })
    ParseAiOutput = {
        jsCode: "// Parse AI output and prepare for content generation\n// MATCHING FORMAT FROM content-generation-flow.json 'Format Form Input'\nconst aiOutput = $input.item.json.output;\nconst context = $('Prepare Daily Context').item.json;\n\nconsole.log('==== PARSE AI OUTPUT (DAILY) ====');\nconsole.log('AI Output:', aiOutput);\n\n// Parse sections with robust regex\nconst sections = {\n  topic: '',\n  headline: '',\n  data_points: '',\n  context: ''\n};\n\n// Extract TOPIC - more flexible matching\nconst topicMatch = aiOutput.match(/---\\s*TOPIC\\s*---\\s*\\n?([\\s\\S]*?)(?=---\\s*HEADLINE|$)/i);\nif (topicMatch) {\n  sections.topic = topicMatch[1].trim().replace(/^[\\-\\*\\•]\\s*/, '');\n}\n\n// Extract HEADLINE - more flexible matching\nconst headlineMatch = aiOutput.match(/---\\s*HEADLINE\\s*---\\s*\\n?([\\s\\S]*?)(?=---\\s*DATA|$)/i);\nif (headlineMatch) {\n  sections.headline = headlineMatch[1].trim().replace(/^[\\-\\*\\•]\\s*/, '');\n}\n\n// Extract DATA_POINTS\nconst dataMatch = aiOutput.match(/---\\s*DATA[_\\s]?POINTS\\s*---\\s*\\n?([\\s\\S]*?)(?=---\\s*CONTEXT|$)/i);\nif (dataMatch) {\n  sections.data_points = dataMatch[1].trim();\n}\n\n// Extract CONTEXT\nconst contextMatch = aiOutput.match(/---\\s*CONTEXT\\s*---\\s*\\n?([\\s\\S]*?)$/i);\nif (contextMatch) {\n  sections.context = contextMatch[1].trim();\n}\n\n// Fallback: if headline is empty, try to extract from topic or generate one\nif (!sections.headline && sections.topic) {\n  // Use first 8 words of topic as headline\n  sections.headline = sections.topic.split(' ').slice(0, 8).join(' ');\n}\n\n// Fallback: if topic is empty, use a default\nif (!sections.topic) {\n  sections.topic = context.custom_topic || 'IT Infrastructure Optimization';\n}\n\nconsole.log('Parsed sections:', sections);\n\n// IMPORTANT: Match EXACT field names used in content-generation-flow.json Format Form Input\n// These are: topic, post_type, visual_style, orientation, headline, data_points, context\n// Plus colors: color_primary, color_secondary, color_accent, color_dark, use_custom_colors\nconst contentRequest = {\n  // Core fields (MUST match Format Form Input expectations)\n  topic: sections.topic,\n  post_type: context.suggested_post_type || 'Educational',\n  visual_style: context.suggested_visual_style || 'Infographic',\n  orientation: '4:5',\n  headline: sections.headline,\n  data_points: sections.data_points,\n  context: sections.context,\n  \n  // MSI Brand Colors (matching Format Form Input defaults)\n  color_primary: '#207CE5',\n  color_secondary: '#004AAD',\n  color_accent: '#FFFDF1',\n  color_dark: '#2B2B2B',\n  use_custom_colors: false,\n  \n  // Metadata for tracking\n  auto_generated: true,\n  generation_date: context.date,\n  day_of_week: context.day_of_week,\n  special_event: context.special_event\n};\n\nconsole.log('Content request prepared (matching Format Form Input):', contentRequest);\nconsole.log('Fields sent: topic, post_type, visual_style, orientation, headline, data_points, context');\nconsole.log('Colors sent: color_primary, color_secondary, color_accent, color_dark');\nconsole.log('==== END PARSE ====');\n\nreturn [{ json: contentRequest }];",
    };

    @node({
        name: 'Save Scheduled Post to DB',
        type: 'n8n-nodes-base.postgres',
        version: 2.5,
        position: [3280, 48],
        credentials: { postgres: { id: 'WnTYs2zvPoE7hDJE', name: 'Postgres account' } },
    })
    SaveScheduledPostToDb = {
        operation: 'executeQuery',
        query: "=INSERT INTO social_posts (\n  topic, \n  post_type, \n  visual_style, \n  orientation, \n  headline, \n  data_points, \n  context, \n  status,\n  created_at, \n  updated_at\n) VALUES (\n  '{{ $json.topic.replace(/'/g, \"''\").replace(/\\n/g, ' ') }}',\n  '{{ $json.post_type.replace(/'/g, \"''\") }}',\n  '{{ $json.visual_style.replace(/'/g, \"''\") }}',\n  '{{ $json.orientation || \"4:5\" }}',\n  '{{ $json.headline.replace(/'/g, \"''\").replace(/\\n/g, ' ') }}',\n  '{{ $json.data_points.replace(/'/g, \"''\").replace(/\\n/g, '\\\\n') }}',\n  '{{ $json.context.replace(/'/g, \"''\").replace(/\\n/g, ' ') }}',\n  'pending',\n  NOW(),\n  NOW()\n)\nRETURNING *",
        options: {},
    };

    @node({
        name: 'Call Content Generation Flow',
        type: 'n8n-nodes-base.httpRequest',
        version: 4.2,
        position: [3504, 48],
    })
    CallContentGenerationFlow = {
        method: 'POST',
        url: 'https://n8nmsi.app.n8n.cloud/webhook/70738d02-4bd8-4dac-853f-ba4836aafaf5',
        sendBody: true,
        specifyBody: 'json',
        jsonBody:
            '={\n  "topic": {{ JSON.stringify($(\'Parse AI Output\').item.json.topic) }},\n  "post_type": {{ JSON.stringify($(\'Parse AI Output\').item.json.post_type) }},\n  "visual_style": {{ JSON.stringify($(\'Parse AI Output\').item.json.visual_style) }},\n  "orientation": "4:5",\n  "headline": {{ JSON.stringify($(\'Parse AI Output\').item.json.headline) }},\n  "data_points": {{ JSON.stringify($(\'Parse AI Output\').item.json.data_points) }},\n  "context": {{ JSON.stringify($(\'Parse AI Output\').item.json.context) }},\n  "color_primary": "#207CE5",\n  "color_secondary": "#004AAD",\n  "color_accent": "#FFFDF1",\n  "color_dark": "#2B2B2B",\n  "use_custom_colors": false\n}',
        options: {
            timeout: 120000,
        },
    };

    @node({
        name: 'Process Generation Response',
        type: 'n8n-nodes-base.code',
        version: 2,
        position: [3728, 48],
    })
    ProcessGenerationResponse = {
        jsCode: "// Process content generation response\nconst response = $input.item.json;\nconst scheduledPost = $('Save Scheduled Post to DB').item.json;\n\nconsole.log('==== PROCESS RESPONSE ====');\nconsole.log('Response:', JSON.stringify(response));\n\nlet postId = scheduledPost.id;\nlet imageUrl = null;\nlet postCopy = null;\n\n// Extract data from response\nif (response.success && response.data) {\n  imageUrl = response.data.image_url || null;\n  postCopy = response.data.post_copy || null;\n  \n  // Use the returned post_id if different\n  if (response.data.post_id) {\n    postId = response.data.post_id;\n  }\n}\n\nconsole.log('Post ID:', postId);\nconsole.log('Image URL:', imageUrl);\nconsole.log('==== END PROCESS ====');\n\nreturn [{\n  json: {\n    scheduled_post_id: scheduledPost.id,\n    generated_post_id: postId,\n    image_url: imageUrl,\n    post_copy: postCopy,\n    success: response.success || false,\n    topic: scheduledPost.topic,\n    headline: scheduledPost.headline,\n    visual_style: scheduledPost.visual_style\n  }\n}];",
    };

    @node({
        name: 'Update Post Status in DB',
        type: 'n8n-nodes-base.postgres',
        version: 2.5,
        position: [3952, 48],
        credentials: { postgres: { id: 'WnTYs2zvPoE7hDJE', name: 'Postgres account' } },
    })
    UpdatePostStatusInDb = {
        operation: 'executeQuery',
        query: "=UPDATE social_posts \nSET \n  status = CASE \n    WHEN '{{ $json.image_url }}' != '' AND '{{ $json.image_url }}' != 'null' THEN 'completed' \n    ELSE 'failed' \n  END,\n  image_url = CASE \n    WHEN '{{ $json.image_url }}' != '' AND '{{ $json.image_url }}' != 'null' THEN '{{ $json.image_url }}'\n    ELSE image_url\n  END,\n  updated_at = NOW()\nWHERE id = '{{ $json.scheduled_post_id }}'\nRETURNING *",
        options: {},
    };

    @node({
        name: 'Format Success Response',
        type: 'n8n-nodes-base.set',
        version: 3.3,
        position: [4176, 48],
    })
    FormatSuccessResponse = {
        assignments: {
            assignments: [
                {
                    id: 'success',
                    name: 'success',
                    value: true,
                    type: 'boolean',
                },
                {
                    id: 'message',
                    name: 'message',
                    value: "=✅ Daily content generated successfully!\n\n📝 Post ID: {{ $json.id }}\n📌 Topic: {{ $json.topic }}\n🎨 Style: {{ $json.visual_style }}\n📰 Headline: {{ $json.headline }}\n🖼️ Image: {{ $json.image_url || 'Pending' }}\n📊 Status: {{ $json.status }}\n⏰ Generated: {{ $json.updated_at }}",
                    type: 'string',
                },
                {
                    id: 'post_id',
                    name: 'post_id',
                    value: '={{ $json.id }}',
                    type: 'string',
                },
                {
                    id: 'image_url',
                    name: 'image_url',
                    value: '={{ $json.image_url }}',
                    type: 'string',
                },
            ],
        },
        options: {},
    };

    @node({
        name: 'Respond to Webhook',
        type: 'n8n-nodes-base.respondToWebhook',
        version: 1.1,
        position: [4400, 48],
    })
    RespondToWebhook = {
        respondWith: 'json',
        responseBody: '={{ { "success": true, "message": "Daily content generated", "data": $json } }}',
        options: {},
    };

    @node({
        name: 'Documentation',
        type: 'n8n-nodes-base.stickyNote',
        version: 1,
        position: [1632, -128],
    })
    Documentation = {
        content:
            '## Daily Content Auto-Generator\n\nThis workflow automatically generates LinkedIn content for MSI Technologies.\n\n### Triggers:\n1. **Scheduled**: Runs daily at 8 AM (Mon-Fri)\n2. **Manual**: POST to `/webhook/daily-content-trigger`\n\n### Manual Trigger Body (optional):\n```json\n{\n  "topic": "Custom topic to generate",\n  "visual_style": "Infographic",\n  "post_type": "Educational",\n  "posts_count": 1\n}\n```\n\n### Flow:\n1. Prepares daily context (date, themes, trends)\n2. AI generates relevant topic and headline\n3. Saves scheduled post to database\n4. Calls main content generation workflow\n5. Updates post with generated image\n\n### Database Integration:\n- Uses `social_posts` table\n- Status flow: `pending` → `completed`/`failed`',
        height: 480,
        width: 400,
        color: 4,
    };

    @node({
        name: 'Trigger Zone',
        type: 'n8n-nodes-base.stickyNote',
        version: 1,
        position: [1950, -68],
    })
    TriggerZone = {
        content: '## 1. TRIGGERS\nSchedule or Manual',
        height: 180,
        width: 260,
        color: 6,
    };

    @node({
        name: 'AI Zone',
        type: 'n8n-nodes-base.stickyNote',
        version: 1,
        position: [2304, -128],
    })
    AiZone = {
        content: '## 2. AI TOPIC GENERATION\nGenerates relevant topic based on date and trends',
        height: 180,
        width: 320,
        color: 5,
    };

    @node({
        name: 'DB Zone',
        type: 'n8n-nodes-base.stickyNote',
        version: 1,
        position: [2896, -128],
    })
    DbZone = {
        content: '## 3. DATABASE & GENERATION\nSave to DB and call content generation',
        height: 180,
        width: 520,
        color: 3,
    };

    @node({
        name: 'Response Zone',
        type: 'n8n-nodes-base.stickyNote',
        version: 1,
        position: [3696, -128],
    })
    ResponseZone = {
        content: '## 4. RESPONSE\nUpdate status and respond',
        height: 180,
        width: 340,
        color: 2,
    };

    @node({
        name: 'Google Gemini Chat Model',
        type: '@n8n/n8n-nodes-langchain.lmChatGoogleGemini',
        version: 1,
        position: [2576, 256],
        credentials: { googlePalmApi: { id: 'oF4jKBK0jRbwEPWt', name: 'Google Gemini(PaLM) Api account' } },
    })
    GoogleGeminiChatModel = {
        options: {},
    };

    // =====================================================================
    // ROUTAGE ET CONNEXIONS
    // =====================================================================

    @links()
    defineRouting() {
        this.DailySchedule8amMonFri.out(0).to(this.MergeTriggers.in(0));
        this.ManualTriggerWebhook.out(0).to(this.MergeTriggers.in(0));
        this.MergeTriggers.out(0).to(this.PrepareDailyContext.in(0));
        this.PrepareDailyContext.out(0).to(this.AiTopicGenerator.in(0));
        this.AiTopicGenerator.out(0).to(this.ParseAiOutput.in(0));
        this.ParseAiOutput.out(0).to(this.SaveScheduledPostToDb.in(0));
        this.SaveScheduledPostToDb.out(0).to(this.CallContentGenerationFlow.in(0));
        this.CallContentGenerationFlow.out(0).to(this.ProcessGenerationResponse.in(0));
        this.ProcessGenerationResponse.out(0).to(this.UpdatePostStatusInDb.in(0));
        this.UpdatePostStatusInDb.out(0).to(this.FormatSuccessResponse.in(0));
        this.FormatSuccessResponse.out(0).to(this.RespondToWebhook.in(0));

        this.AiTopicGenerator.uses({
            ai_languageModel: this.GoogleGeminiChatModel.output,
        });
    }
}
